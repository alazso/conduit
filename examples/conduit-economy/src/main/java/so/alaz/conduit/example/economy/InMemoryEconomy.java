...
/**
 * A minimal in-memory {@link Economy} provider used to exercise the full Conduit
 * stack on a development server. Balances live in memory only (reset on restart),
 * which is perfect for testing and obviously not for production.
 */
public final class InMemoryEconomy implements Economy {

    private final Currency currency = SimpleCurrency.ofDefault("dollars", "$", 2);
    private final Map<UUID, BigDecimal> balances = new ConcurrentHashMap<>();

    @Override
    public @NotNull String getName() {
        return "ConduitExampleEconomy";
    }
